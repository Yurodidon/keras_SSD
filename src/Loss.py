...
class MultiboxLoss(object):
    ...
    def _softmax_loss(self, y_true, y_pred):
        ...
        """
        y_pred = tf.maximum(tf.minimum(y_pred, 1), 1e-15)
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        # loss for all priors
        conf_loss = self._softmax_loss(y_true[:, :, 4:-8], # conf
                                       y_pred[:, :, 4:-8])
        loc_loss = self._l1_smooth_loss(y_true[:, :, :4], # loc
                                        y_pred[:, :, :4])

        # get positives loss

        # y_true[:, :, -8] mean is there a object in this box
        # y_true[:, :, -8] == 1 -> there is a object in box, otherwise -> no object
        num_pos = tf.reduce_sum(y_true[:, :, -8], axis=-1)
        # filter out all the conf & loc which is not exist
